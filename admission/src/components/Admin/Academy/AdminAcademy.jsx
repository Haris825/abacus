...
function AdminAcademy() {
  ...
  useEffect(() => {
    fetch('http://localhost:5049/admin/viewInstitute')
  ...
  const handleDelete = (instituteId) => {
    fetch(`http://localhost:5049/admin/deleteInstitute/${instituteId}`, {
      method: 'DELETE',
    })
      .then((response) => response.json())
      .then((data) => {
      ...
      <div className="container">
        <div className="row">
          {institutes.map((institute) => (
            <div className="col-md-4" key={institute.id}>
              <div className="card mb-3">
                ...
                <div className="card-body">
                  <h3 className="card-title" style={{ marginBottom: '5px' }}>
                    {institute.academyName}
                  </h3>
                  <p className="card-text" style={{ marginBottom: '5px' }}>
                    Contact: {institute.contactNumber}
                  </p>
                  <p className="card-text" style={{ marginBottom: '5px' }}>
                    Email: {institute.emailId}
                  </p>
                  <p className="card-text" style={{ marginBottom: '5px' }}>
                    Location: {institute.academyLocation}
                  </p>
                </div>
                <div className="card-actions">
                  <div className="d-flex justify-content-between">
                  <Link to={`/editInstitute/${institute.id}`}>
                      <FaPencilAlt size={20} />
                    </Link>
                    <button
