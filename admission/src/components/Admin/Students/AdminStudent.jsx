...
function AdminStudent() {
  ...
  useEffect(() => {
    fetch('http://localhost:5049/admin/viewStudent')
  ...
  const handleDelete = (studentId) => {
    fetch(`http://localhost:5049/admin/deleteStudent/${studentId}`, {
      method: 'DELETE',
    })
      .then((response) => response.json())
      .then((data) => {
    ...
    <div><br/><br/>
      ...
      {students.map((student) => (
        <div key={student.id} className="student-row">
          <div className="student-item">{student.id}</div>
          <div className="student-item">{`${student.firstName} ${student.lastName}`}</div>
          <div className="student-item">{student.courseEnrolled}</div>
          <div className="student-item">{student.phoneNumber1}</div>
          <div className="student-item">
            <span className="action-icons">
              <Link to={`/editStudent/${student.id}`} className="edit-icon">
                <FaEdit />
              </Link>
              <FaTrash className="trash-icon" onClick={() => handleDelete(student.id)} />
            </span>
          </div>
        </div>
      ))}
